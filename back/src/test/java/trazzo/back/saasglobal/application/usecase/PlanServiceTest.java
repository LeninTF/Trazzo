...
class PlanServiceTest {
    ...
    private static Plan plan(int id) {
        ...
        return Plan.restore(id, "Basic", BigDecimal.valueOf(99), BigDecimal.valueOf(999),
                "SOLES", "MONTHLY", true, now, now, null);
    }
    ...
    @Test
    void create_withFeatures_savesBooleanAndNumeric() {
        when(planRepository.save(any())).thenReturn(plan(1));
    ...
    void listActive_returnsMappedResults() {
        ...
        var results = service.listActive();

        assertEquals(1, results.size());
        assertEquals("Basic", results.get(0).name());
    }

    @Test
    void listAll_returnsMappedResults() {
        when(planRepository.findAll()).thenReturn(List.of(plan(1), plan(2)));
        when(planFeatureRepository.findByPlanId(anyInt())).thenReturn(List.of());
    ...
    void activate_savesAndReturnsActivated() {
        var now = LocalDateTime.now();
        var inactive = Plan.restore(1, "Basic", BigDecimal.valueOf(99), BigDecimal.valueOf(999),
                "SOLES", "MONTHLY", false, now, now, null);
    ...
    @Test
    void deleteById_savesDeletedPlan() {
        when(planRepository.findById(1)).thenReturn(Optional.of(plan(1)));
        when(planRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
    ...
    @Test
    void getById_mapsPersistedFeaturesToTypedValues() {
        when(planRepository.findById(1)).thenReturn(Optional.of(plan(1)));
        when(featureRepository.findAll()).thenReturn(List.of(
                feature(10, "max_trabajadores"), feature(11, "reportes")));
        when(planFeatureRepository.findByPlanId(1)).thenReturn(List.of(
                PlanFeature.restore(1, 1, 10, "INT", "250", java.time.LocalDate.now(), null, true, LocalDateTime.now(), LocalDateTime.now()),
                PlanFeature.restore(2, 1, 11, "BOOLEAN", "true", java.time.LocalDate.now(), null, true, LocalDateTime.now(), LocalDateTime.now())));
