...
public class AuditMetricsService implements AuditMetricsUseCase {
    ...
    public AuditMetricsResult getMetrics() {
        ...
        Long sesionesActivas = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sesion WHERE state = 'ACTIVE'", Long.class);

        Long totalSesiones = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sesion", Long.class);
