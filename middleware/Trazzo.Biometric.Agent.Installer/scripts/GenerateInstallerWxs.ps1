...
foreach ($file in $files) {
    ...
    if ($relativePath -ieq "Trazzo.Biometric.Agent.exe") {
        ...
        # Recuperación automática: 1er/2do/3er fallo → reinicio tras 10 s; contador resetea cada 24 h.
        $utilNs = "http://wixtoolset.org/schemas/v4/wxs/util"
        $serviceConfig = $xml.CreateElement("util", "ServiceConfig", $utilNs)
